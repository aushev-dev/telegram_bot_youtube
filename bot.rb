...
Telegram::Bot::Client.run(token) do |bot|
  bot.listen do |message|
    begin
	    case message.text
	  	when /yout/
	  		%x(/app/youtube-dl.dms -x --audio-format mp3 --output "audio.%(ext)s" -o "/temp/audio.mp3" #{message.text})
			bot.api.send_audio(chat_id: 64819429, audio: Faraday::UploadIO.new('/temp/audio.mp3', 'audio/mp3'))
		when //
